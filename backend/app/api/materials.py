...
async def delete_material(
    ...
    Prevents deletion if there are active (non-expired) shared links.
    Users must wait for links to expire naturally before deleting the material.
    """
    from app.models.shared_link import SharedLink
    from datetime import datetime
    ...
    # Check for active, non-expired shared links
    now = datetime.utcnow()
    active_links = db.query(SharedLink).filter(
        SharedLink.material_id == material_id,
        SharedLink.is_active == True,
    ...
    if active_links > 0:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Cannot delete material: {active_links} active shared link(s) still exist. Please wait for the links to expire before deleting this material."
        )
